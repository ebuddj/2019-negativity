  ...
  render() {
    ...
    return (
      <div className={style.app}>
       <h1 className={style.header}>What is country's problem?</h1>
          ...
          {items.map(item => (
            <div key={item.name} className={style.radar_container}>
              ...
              <RadarChart cx={60} cy={60} outerRadius={60} width={120} height={120} data={item.data} domain={0,100}>
                <PolarGrid />
                <Tooltip formatter={(value, name, props) => { return [props.payload.value_real] }} cursor={false} wrapperStyle={{backgroundColor:'transparent', zIndex: 20000}} contentStyle={{backgroundColor:'transparent', border:0}}/>
                <PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} />
